...
double mHH=0., cosHH=0., weight=0., mHHwidth=0., cosHHwidth=0.;
...
void SubJetSoftDropped_DM(const int pu, const int nmin, const int nmax, const char *output)
{
  ...
  TH2D *M_vs_cos	    = new TH2D("h_m_vs_cos"    ,";m_{HH};cos#theta;;",100, 0, 2000, 100, -1.0, 1.0);
	  ...
	  h_events->Fill(5);
	    ...
	    if( p4_ak8jet0.M() > 90 && p4_ak8jet0.M() < 140 &&
 		...
 		h_cutflow_2->Fill(8);
	      ...
	      if((sj00BTagged+sj01BTagged+sj10BTagged+sj11BTagged)==3){
 	        h_cutflow_2->Fill(10);
                M_HH_3b->Fill((p4_ak8jet0+p4_ak8jet1).M(), weight);
		...
		M_HH_3b->SetLineWidth(3);
	      ...
	      if((sj00BTagged+sj01BTagged+sj10BTagged+sj11BTagged)==4){
 	        h_cutflow_2->Fill(11);
                M_HH_4b->Fill((p4_ak8jet0+p4_ak8jet1).M(), weight);
